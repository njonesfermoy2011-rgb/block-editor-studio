...
**Batch D dropped (verified 2026-06-29):** `wp.blocks.pasteHandler` in WP 7.0 already strips Word/Google-Docs junk cleanly (Google-Docs spans → `Hello <strong>bold</strong> world`; Word `MsoNormal`/`mso-*`/`<o:p>` → clean `<p><strong>…</strong></p>`), so F6 paste cleanup would duplicate core and risk interfering with it. F3 link defaults has no sanctioned hook (the "extend link popover" request #11599 is still open); a plugin could only inject defaults via fragile post-hoc interception, against our no-fragile-DOM principle. Net: the suite is complete at **v0.7.0**. (Possible clean replacement if more value is wanted: an **autosave/Heartbeat interval toggle** — low risk via `heartbeat_settings`, real demand from slow-host users.)
